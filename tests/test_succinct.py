...
from succinct import Succinct, _find_ith_one, bitarray

_P = NamedTuple("_P", [("pos", int), ("val", bool)])
...
class TestSuccinctBasis:
    def test_bitarray_get_overflow(self):
        bits = bitarray()

        assert bits[42] == 0

    @pytest.mark.parametrize("bits01, point, expected01", BASIC_TEST_DATA1)
    def test_set_bit(self, bits01: str, point: _P, expected01: str):
        bits = bitarray(bits01)
        bits[point.pos] = point.val
...
class TestSuccinct:
    def test_from_list(self, domains: list[str], leaf_map01: str, label_map01: str, labels: str):
        trie = Succinct.from_iterable(domains)

        assert trie.leaf_map.to01() == leaf_map01
        assert trie.label_map.to01() == label_map01
        assert trie.labels == labels

    def test_to_list(self, domains: list[str], leaf_map01: str, label_map01: str, labels: str):
        trie = Succinct(
            leaf_map=bitarray(leaf_map01), label_map=bitarray(label_map01), labels=labels
        )

        assert [*trie.keys()] == domains
